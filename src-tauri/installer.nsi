...
; template padrão deixa a caixa "criar atalho" MARCADA. Trocar isso exige
; apontar `nsis.template` para um arquivo nosso — este.
;
...
;
; MUDANÇA 1 — a caixa "criar atalho na área de trabalho" nasce DESMARCADA.
; MUDANÇA 2 — a instalação passiva NÃO cria o atalho. É por ela que a
...
; [KYBERFOOD] mudança 1: a caixa nasce DESMARCADA (o padrão do MUI é marcada).
; A opção continua na tela — quem quiser o atalho marca; quem não olhar, não ganha.
...
Section Install
  ...
  check_ns_flag:
    ...
    IfErrors 0 shortcuts_done
      ; [KYBERFOOD] mudança 2: sem `Call CreateDesktopShortcut` aqui. Este é o
      ; caminho da instalação silenciosa/passiva, por onde passa a ATUALIZAÇÃO
      ; AUTOMÁTICA do app — criar o atalho aqui o traria de volta a cada versão.
      Call CreateStartMenuShortcut
  shortcuts_done:
